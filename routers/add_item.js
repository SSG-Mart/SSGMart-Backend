...
router.use((req,res,next) => {
  ...
  con.query(sql_get_seller_id, (err, result) => {
    if(err){
      console.log(err);
      res.send({status:500, data:{type: 'error', msg:'Database error', color:'red'}});
    }
    else{
      if(result.length > 0){
        ...
        res.send({status:404, data:{type: 'error', msg:'Not Found', color:'red'}});
      }
    }
  })
})
...
router.post("/", (req, res) => {
  ...
  if(title && category_id && quantity && image && unit && sub_category_id && unit_price && description, time_period){
    ...
    res.send({ status: 500, data: { type: 'error', msg: "fill all data", color: "red"}});
  }
});
...
router.post("/getcategory", (req, res) => {
  ...
  con.query(sql, (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        const {name} = result[0];
        res.send([{id: mainCategoryID, name: name}, {list_of_sub_category}]);
      } else {
        res.send({ status: 404, data: {type: 'error', msg: "Not Found", color: "red"} });
      }
    }
